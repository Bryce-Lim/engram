...
``connect_engram`` gives you an :class:`AgentDriver` talking to an in-process
:class:`~engram.proxy.Engram`, which in turn drives the same mock server
subprocess. The proxy runs in a daemon thread reading a real OS pipe, so the
byte path is identical to production ``engram wrap`` — only the transport
...
from engram.predictors.cot_oracle import CoTOracle, IntentRule  # noqa: E402
from engram.proxy import Engram  # noqa: E402
...
def connect_baseline() -> Tuple[AgentDriver, "subprocess.Popen"]:
    """Agent talks directly to the mock server subprocess (no Engram)."""
    import subprocess
...
class EngramHandle:
    """Owns an in-process Engram proxy and the agent connected to it."""

    def __init__(self, proxy: Engram, driver: AgentDriver,
                 agent_to_engram: _Pipe, engram_to_agent: _Pipe,
                 thread: threading.Thread) -> None:
        self.proxy = proxy
        self.driver = driver
        self._a2p = agent_to_engram
        self._p2a = engram_to_agent
        self._thread = thread

    def close(self) -> None:
        # Closing the agent->engram write end gives Engram's read loop EOF.
        try:
...
def connect_engram(on_log=None, install_demo_rules: bool = True,
                   markov_min_observations: int = 1) -> EngramHandle:
    """Agent talks to an in-process Engram that drives the mock server."""
    a2p = _Pipe()  # agent writes -> engram reads
    p2a = _Pipe()  # engram writes -> agent reads
    ...
    thread = threading.Thread(target=proxy.serve_forever, name="engram-serve")
    thread.daemon = True
    thread.start()

    driver = AgentDriver(write_stream=a2p.writer, read_stream=p2a.reader)
    return EngramHandle(proxy, driver, a2p, p2a, thread)
